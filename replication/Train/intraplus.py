...
def preprocess(path,test_id,train_id, with_args_num=True, with_pac_depth=True):
    ...
    return method[train_id], exception[train_id], sentences_train, sentences_test, features_train, features_test, refers_train, refers_test, \
...
for i in range(train_samples.shape[0]):
    if (i%10!=0):
        Ztrain_samples.append(train_samples[i])
        Zlabels_train.append(labels_train[i])
    else:
        Ztest_samples.append(test_samples[i])
train_samples =Ztrain_samples
labels_train =Zlabels_train
test_samples =Ztest_samples
# idx = int(train_samples.shape[0] * 0.9)
...
model_RandomUnderSample = RandomUnderSampler(sampling_strategy=0.9)
...
trainData='trainpre.csv'
testData ='testpre.csv'
...
predictor = task.load('agModels-predictClass_KXM') # unnecessary, just demonstrates how to load previously-trained predictor from file
train_data = task.Dataset(file_path=trainData)
test_data = task.Dataset(file_path= testData)

# TODO
predictor = task.fit(train_data=train_data, label='test', output_directory=dir,auto_stack=True,time_limits = 600)
...
print("intraplus", mapping[t_id])
print("Predictions:  ", y_pred)
print("Predictions:  ", predictor.predict_proba(test_data_nolab))
...
#Predict_28 = [(mapping[i],accuracy,precision,recall,f1)]
#Predict_28 = pd.DataFrame(Predict_28)
#Predict_28.to_csv('Predict.csv',mode='a',index=False,header=False)
...
for i in range(features_train.shape[0]):
    ...
    if method_to_link.get(method[i]) is not None:
        ...
        for idx, train_method in enumerate(split):
            if train_method:
                new_fea = np.array(features_train[i], copy=True)
                new_fea[1] = train_method.count('.')
                new_fea[2] = train_method.count('@')
                new_fea[3] = idx
                new_fea[4] = len(split) - idx

                conc = np.concatenate([sentences_train[i], new_fea, refers_train[i], abstract_train[i], np.zeros(shape=(1,))], axis=0).T
        ...
        con = np.concatenate([sentences_train[i], features_train[i], refers_train[i], abstract_train[i], np.zeros(shape=(1,))],axis=0)
